...
func redirectTo(t *testing.T, rawURL string) *http.Request {
	t.Helper()
...
func TestNotifierRedirectGuard_BlocksByPolicy(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := guard(redirectTo(t, tc.target), nil)
			if tc.blocked {
...
func TestNotifierRedirectGuard_CapsHops(t *testing.T) {
	...
	err := guard(redirectTo(t, "http://8.8.8.8/x"), via)
...
func TestNotifierRedirectGuard_NilConfigOnlyCaps(t *testing.T) {
	...
	require.NoError(t, guard(redirectTo(t, "http://127.0.0.1/x"), nil))
	// ...but the hop cap still applies.
	assert.Error(t, guard(redirectTo(t, "http://127.0.0.1/x"), make([]*http.Request, maxNotifierRedirects)))
}
